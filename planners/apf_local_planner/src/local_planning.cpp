...
void Local_Planner::init(ros::NodeHandle& nh)
{
    ...
    odom_sub = nh.subscribe("/odom",10,&Local_Planner::odom_cb, this);
    ...
    command_pub_ = nh.advertise<quadrotor_msgs::PositionCommand>("/planning/pos_cmd",10);
    // 发布提示消息
...
void Local_Planner::drone_state_cb(const prometheus_msgs::DroneStateConstPtr& msg)
{
    ...
    Drone_odom.child_frame_id = "base";
...
void Local_Planner::odom_cb(const nav_msgs::OdometryConstPtr &odom)
{
...
void Local_Planner::control_cb(const ros::TimerEvent& e)
{
    ...
    Command_Now.Reference_State.yaw_ref             = desired_yaw;
    position_cmd.velocity.x = desired_vel[0];
    position_cmd.velocity.y = desired_vel[1];
    position_cmd.velocity.z = desired_vel[2];
    command_pub_.publish(position_cmd);
    command_pub.publish(Command_Now);
